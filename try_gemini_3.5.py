...
# Note: The prompt asks for a strict word-for-word transcript.
...
for i, key in enumerate(GEMINI_KEYS):
    try:
        print(f"\nTrying Gemini Key {i+1} with gemini-3.5-flash...")
        ...
        with open(AUDIO_FILE, "rb") as f_audio:
            audio_file = client.files.upload(
                ...
                config=types.UploadFileConfig(
                    mime_type="audio/mp3",
                    display_name="bhavishya_malika_puran_35"
                )
            )
        ...
        print("Generating transcript with gemini-3.5-flash...")
        ...
        with open(gemini_out, "w", encoding="utf-8") as f:
            f.write("=" * 60 + "\n")
            f.write("GEMINI 3.5 FLASH TRANSCRIPT\n")
            f.write(f"File : bhavishya malika puran (Manish Bhaiya)-2606061818.mp3\n")
            f.write(f"Model: gemini-3.5-flash | Key: {i+1}\n")
            f.write("=" * 60 + "\n\n")
