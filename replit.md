...
**Production deployment**: The API server artifact.toml build step runs `pnpm install --frozen-lockfile` (which triggers `uv sync` via postinstall and downloads the `ffmpeg-static` binary) before bundling with esbuild. Environment vars `UV_PROJECT_ENVIRONMENT=/home/runner/workspace/.pythonlibs`, `UV_PYTHON_PREFERENCE=only-system`, and `UV_PYTHON_DOWNLOADS=never` ensure Python deps go to the correct location. The PYTHONPATH in spawn calls (`youtube.ts`, `bhagwat.ts`) is set dynamically from `process.env.REPL_HOME ?? process.cwd()` so it works in both dev and production.

**FFmpeg**: FFmpeg is provided by the `ffmpeg-static` npm package (a static pre-compiled binary). It is added to `onlyBuiltDependencies` in `pnpm-workspace.yaml` so its install script runs during `pnpm install`. All three route files (`youtube.ts`, `bhagwat.ts`, `subtitles.ts`) pass `--ffmpeg-location <path>` to yt-dlp so it always finds FFmpeg regardless of OS/environment. The package is externalized in `build.mjs` so the binary stays in `node_modules` and the path resolves correctly at runtime.
