...
**Models:** Studio Copilot exposes two public modes backed by a measured multi-provider ladder. **Ultra** and **Fast** both start with Mistral Small while retaining separate prompts and tool budgets. The preferred head of the fallback ladder (`getCopilotFallbackModels`, minus whichever route is already primary) is Mistral Small → AgentRouter `gpt-5.6-sol` → Mistral Medium → Ollama GPT-OSS. Models proven with 30K+ transcript inputs follow in measured order (Ollama GPT-OSS, Mistral Small, Mistral Medium, Devstral, Mistral Large, SambaNova GPT-OSS, NVIDIA Nemotron Super, NVIDIA GPT-OSS). Short-input or quota-limited emergency routes follow through Groq, OpenRouter, Kilo, Aion, SambaNova DeepSeek, and Nemotron Ultra. Provider-qualified route IDs prevent identical upstream model names from being confused. Each request has a 30-second first-chunk/stream-idle timeout; keys rotate before model fallback. Gemini remains internal for native media/vision helper calls. Max iterations default is **49** (`COPILOT_MAX_ITERATIONS` ?? `"49"`).
...
| `MISTRAL_API_KEY` | ✅ (Copilot fallback) | — | Mistral Medium/Small/Large/Devstral credential |
| `SAMBANOVA_API_KEY` | Optional | — | SambaNova GPT-OSS and DeepSeek fallback credential |
| `OPENROUTER_API_KEY` | Optional | — | OpenRouter fallback credential |
| `AION_API_KEY` | Optional | — | AionLabs emergency fallback credential |
| `KILO_API_KEY` | Optional | — | Kilo Gateway emergency fallback credential |
| `COPILOT_ULTRA_MODEL` | | `mistral:mistral-small-latest` | Ultra/default provider-qualified route; mode-specific prompting remains separate |
| `COPILOT_FAST_MODEL` | | `mistral:mistral-small-latest` | Fast provider-qualified route; AgentRouter `gpt-5.6-sol` and Ollama GPT-OSS follow in the fallback ladder |
