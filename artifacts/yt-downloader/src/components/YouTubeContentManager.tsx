...
export function YouTubeContentManager() {
  ...
  }, [activeTopic, generateThoughts, pack, searches, selected?.name, selectedId, summary]);
  ...
  return (
    <div className="ytcm-page ytcm-chat-shell">
      <header className="ytcm-chat-topbar">
        <div className="ytcm-chat-brand">
          <span className="ytcm-mark"><Youtube className="w-4 h-4" /></span>
          <div>
            <span>Content Manager</span>
            <p>{selected ? `${selected.name} - ${selected.videoCount} videos in memory` : "Select a saved channel before sending"}</p>
          </div>
        </div>
        <div className="ytcm-chat-controls">
          <button type="button" className="ytcm-round-btn" title="New chat" onClick={startNewChat}>
          ...
          <div className="ytcm-history-wrap">
            <button
              ...
              title="History"
              onClick={() => setShowHistoryDropdown((value) => !value)}
            >
            ...
            {showHistoryDropdown ? (
              <div ref={historyDropdownRef} className="ytcm-history-menu">
                ...
                <div className="ytcm-history-list">
                  {sessions.length === 0 ? (
                    <div className="ytcm-history-empty">No chats yet</div>
                    ...
                    >
                      ...
                      <em>{session.turns.length} turns</em>
                      <i onClick={(event) => deleteSession(session.id, event)}><Trash2 className="w-3 h-3" /></i>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <select value={selectedId} onChange={(event) => setSelectedId(event.target.value)} title="Selected channel">
            <option value="">No channel selected</option>
            {profiles.map((profile) => (
              <option key={profile.id} value={profile.id}>{profile.name}</option>
            ))}
          </select>
          <button type="button" className="ytcm-round-btn" title="Refresh channels" disabled={profilesLoading} onClick={() => void refreshProfiles()}>
            <RefreshCw className={cn("w-4 h-4", profilesLoading && "animate-spin")} />
          </button>
          <button type="button" className="ytcm-round-btn" title="Add channel" onClick={() => { setPendingScrapeId(null); setChannelInput(""); setConfirmOpen(true); }}>
            <Youtube className="w-4 h-4" />
          </button>
          <button type="button" className="ytcm-round-btn" title="Delete selected channel" disabled={!selectedId} onClick={() => selectedId && void removeProfile(selectedId)}>
            <Trash2 className="w-4 h-4" />
          </button>
          <label className="ytcm-live-pill">
            <input type="checkbox" checked readOnly />
            <span><Globe className="w-3.5 h-3.5" /> Tavily</span>
          </label>
        </div>
      </header>
...
function CompletedTurn({
  ...
  return (
    <>
      ...
      {turn.thoughts.trim() || turn.searches.length > 0 ? (
        <ThoughtStatusBlock
