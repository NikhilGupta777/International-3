...
function sanitizeSearchEntryPoint(html: string): string {
  ...
  template.content.querySelectorAll("script, iframe, object, embed, form, input, button, style, svg, math, link, meta, base, template").forEach(el => el.remove());
...
export function StudioCopilot({
  ...
  const sendMessage = useCallback(async (text: string, attachmentsArg?: Array<{ type: string; name: string; mimeType: string; data?: string; url?: string; previewUrl?: string }>) => {
    ...
    const handleEvent = (evt: SseEvent) => {
      ...
      if (evt.type === "navigate") { onNavigateRef.current?.(evt.tab); return; }
  ...
  }, [streaming, reasoningMode, BASE, updateSession, ensureSession, upsertMsg, consumeLeadingSkillCommand, resetComposerInput]);
